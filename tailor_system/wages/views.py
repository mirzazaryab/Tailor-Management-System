...
from .models import Order, OrderItem, Customer, Tailor, Product, WorkRecord, DailyWork, OrderPayment
...
def dashboard(request):
    ...
    recent_records = DailyWork.objects.select_related('tailor', 'product', 'customer').order_by('-date', '-id')[:7]
...
def add_product(request):
    ...
    if products_page_obj:
        for i, product in enumerate(products_page_obj):
            print(
                f"Product {i + 1} on page: ID={product.id}, Name='{product.name}', Customer Price='{product.customer_price}', Tailor Rate='{product.tailor_payment_rate}'")
    else:
...
def record_work(request):
    ...
    if request.method == 'POST':
        ...
        if not all([tailor_id, product_id, qty_str, work_date]):
            messages.error(request, 'Tailor, Product, Quantity, and Date are required fields.')
...
def daily_report(request):
    ...
    # Fetch WorkRecord Records (Order-specific Work)
    order_work_records_queryset = WorkRecord.objects.filter(
        date=selected_date,
        is_order_work=True
    ).select_related('tailor', 'product', 'order_item__order__customer')
    ...
    if tailor_id:
        try:
            ...
            order_work_records_queryset = order_work_records_queryset.filter(tailor=selected_tailor)
    ...
    order_work_total = order_work_records_queryset.aggregate(
        total_value=Sum(F('quantity') * F('rate'))
    )['total_value'] or Decimal('0.00')

    grand_total_earnings = daily_total + order_work_total

    total_items_produced = (daily_work_records_queryset.aggregate(total_qty=Sum('quantity'))['total_qty'] or 0) + \
                           (order_work_records_queryset.aggregate(total_qty=Sum('quantity'))['total_qty'] or 0)

    context = {
        'daily_work_records': daily_work_records_queryset,
        'order_work_records': order_work_records_queryset,
        'tailors': tailors,
        'selected_tailor': selected_tailor,
        'selected_tailor_id': tailor_id,
        'date': selected_date.isoformat(),
        'daily_total': daily_total,
        'order_work_total': order_work_total,
